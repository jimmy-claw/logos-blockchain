...
When running a node locally with a custom deployment config, you may encounter the following error if the 
...
Once updated, remove the `db` folder and restart the node.
